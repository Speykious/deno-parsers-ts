import { ParserState, ErrorMsgProvider } from './ParserState.ts';
...
export class Parser<TOut> {
	...
	map<T>(fn: (result: TOut) => T) {
		return new Parser<T>(inputState => {
			...
			return nextState.resultify(fn(nextState.result as TOut));
		});
	}
	...
	filter(fn: (result: TOut) => boolean, filteringEMP: ErrorMsgProvider) {
		return new Parser<TOut>(inputState => {
			...
			if (!fn(nextState.result as TOut))
				return nextState.update(inputState.index, null).errorify(filteringEMP);
	...
	chain<T>(fn: (result: TOut) => Parser<T>) {
		return new Parser<T>(inputState => {
			...
			const nextParser = fn(nextState.result as TOut);
